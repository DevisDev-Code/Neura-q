...
import { performMarketResearch, generateDebateTurn, generateSynthesis } from './services/geminiService';
import { AppState, DebateMessage, IntakeData, Phase, AgentRole } from './types';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen font-sans text-text bg-bg selection:bg-secondary selection:text-white">
      ...
      <div className="fixed bottom-0 left-0 w-full border-t border-border bg-bg/90 backdrop-blur-sm py-2 px-6 flex justify-between items-center text-[10px] font-mono tracking-widest text-muted z-50">
        <div>NEURA-Q · ENGINE V2.0</div>
        <div className="flex gap-4">
           <span className={phase === 'intake' ? 'text-secondary' : ''}>01 SITUATION</span>
           <span className={phase === 'research' ? 'text-secondary' : ''}>02 INTEL</span>
           <span className={phase === 'debate' ? 'text-secondary' : ''}>03 WAR ROOM</span>
           <span className={phase === 'synthesis' ? 'text-secondary' : ''}>04 VERDICT</span>
        </div>
